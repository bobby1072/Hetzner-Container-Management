...
{
    private readonly Channel<KeyValuePair<Guid, (InfrastructureComponentUpdateInput[] Input, Func<Guid, InfrastructureComponent[]?, ApiException?, CancellationToken, Task>? AddToCompleteQueueFunc)>> _updateOperationChannel = 
        Channel.CreateUnbounded<KeyValuePair<Guid, (InfrastructureComponentUpdateInput[] Input, Func<Guid, InfrastructureComponent[]?, ApiException?, CancellationToken, Task>? AddToCompleteQueueFunc)>>();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await QueueUpdateOperation(input, 
            (id, ifd,ex, ct) => completeQueue.Writer.WriteAsync(new KeyValuePair<Guid, (InfrastructureComponent[]? Response, ApiException? Exception)>(id, (ifd, ex)), ct).AsTask(),
            cancellationToken);
